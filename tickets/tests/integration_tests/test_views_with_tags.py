...
class ProjectTaggingTestCase(TestCase):
    ...
    def test_tags_in_snippet_details_view(self):
        ...
        for tag in tags:
            tag_url = reverse('tickets_tagged_with', args=(tag, ))
    ...
    def test_tags_ticket_list_view(self):
        ...
        tags_back = self.ticket1.tags.all().order_by("name")
        self.assertQuerysetEqual(tags_back, tags,
                                 lambda a: str(a.name))
        ...
        # not ticket 3
        response = self.client.get(reverse('tickets_tagged_with',
                                           args=(tags[0],)), user=self.user1)
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(reverse('ticket_list'), user=self.user1)
        self.assertEqual(response.status_code, 200)
