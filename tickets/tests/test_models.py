...
class TestTicket(TestCase):
    ...
    def test_ticket_get_url(self):
        ...
        url = "/tickets/{0}/".format(self.ticket1.id)
        self.assertIn(url, self.ticket1.get_absolute_url())
        url = "/tickets/{0}/".format(self.ticket2.id)
...
class TestTicketParentChildren(TestCase):
    ...
    def test_ticket_get_children(self):
        ...
        self.assertQuerysetEqual(children, shouldbe, lambda a: a.id, ordered=False)
...
class TestTicketDuplicates(TestCase):
    ...
    def test_ticket_duplicate_str(self):
        ...
        shouldbe = "Ticket {} is a duplicate of ticket {}".format(second.id, first.id)
    ...
    def test_get_duplicate_and_original_tickets(self):
        ...
        self.assertEqual(self.ticket2.get_originals()[0].original.id, self.ticket1.id)
        self.assertEqual(self.ticket1.get_duplicates()[0].ticket.id, self.ticket2.id)
...
class TestCommentManager(TestCase):
    ...
    def setUp(self):
        ...
        self.comment3 = FollowUpFactory(ticket=self.ticket1, private=True)
