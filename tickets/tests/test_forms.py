...
class TestCloseTicketForm(TestCase):
    ...
    def test_duplicate_good_data(self):
        ...
        self.assertTrue(form.is_valid())
        # check the data
        self.assertEqual(form.cleaned_data["comment"], "A valid comment")
    ...
    def test_duplicate_missing_ticket(self):
        ...
        initial = {"comment": "This is a valid comment", "duplicate": True}
        form = CloseTicketForm(
            data=initial,
            instance=self.comment,
            ticket=self.ticket,
...
class TestSplitForm(TestCase):
    ...
    def test_good_data(self):
        ...
        form = SplitTicketForm(
            data=initial, user=self.user, original_ticket=self.ticket
        )
        ...
        self.assertEqual(
            form.cleaned_data["ticket_type1"], str(self.ticket.ticket_type)
        )
        self.assertEqual(form.cleaned_data["priority1"], str(self.ticket.priority))
        self.assertEqual(form.cleaned_data["assigned_to1"], self.ticket.assigned_to)

        self.assertEqual(form.cleaned_data["application1"], self.ticket.application)
        self.assertEqual(form.cleaned_data["application2"], self.ticket.application)

        self.assertEqual(form.cleaned_data["title1"], self.ticket.title)
        self.assertEqual(form.cleaned_data["title2"], self.ticket.title)

        self.assertEqual(form.cleaned_data["description1"], self.ticket.description)
        self.assertEqual(form.cleaned_data["status2"], "new")
        self.assertEqual(
            form.cleaned_data["ticket_type2"], str(self.ticket.ticket_type)
        )
        self.assertEqual(form.cleaned_data["priority2"], str(self.ticket.priority))
        self.assertEqual(form.cleaned_data["assigned_to2"], self.ticket.assigned_to)
        self.assertEqual(form.cleaned_data["description2"], self.ticket.description)
